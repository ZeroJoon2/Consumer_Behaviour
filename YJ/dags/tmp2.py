...
from tmp1 import search_videos, get_all_comments, save_comments_to_parquet, send_failure_email, send_success_email
import logging
...
def save_comments_task(search_query, parquet_filename, **kwargs):
    all_comments = kwargs['ti'].xcom_pull(key=f'all_comments_{search_query.replace(" ", "_")}', task_ids=f'comment_collection_task_{search_query}')
    save_comments_to_parquet(all_comments, filename=parquet_filename)

# DAG 정의
# 매달 1일에 실행하도록 일단 설정!
with DAG('tmp_dag', default_args=default_args, schedule_interval=None) as dag:
